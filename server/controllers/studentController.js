...
const generateTest = async (req, res) => {
  try {
    ...
    // Assessment mode now uses the CAT engine (routed via /api/student/cat/start)
    // This static path is no longer used for assessments.
    if (req.body.testType === 'assessment') {
      return res.status(400).json({
        message: 'Assessments now use the adaptive CAT engine. Please use the CAT session endpoint.'
      });
    }
